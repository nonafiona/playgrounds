...
class Board {
    ...
    func checkMovement(direction: Direction, orientation:Orientation) -> Bool {
        ...
        var lastMergeableTile: Tile? = nil
        for i in 0..<config.tileNumber {
            lastZeroTile = nil
            lastMergeableTile = nil
            for j in 0..<config.tileNumber {
                let temp = direction == .forward ? (config.tileNumber - 1) - j : j
                let x = orientation == .horizontal ? temp : i
                let y = orientation == .horizontal ? i : temp
                let tile = tileList[x + y * config.tileNumber]
                if !tile.isEmpty {
                    if let mergableTile = lastMergeableTile, mergableTile.value == tile.value {
